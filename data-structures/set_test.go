...
func TestSet_Filter(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "nonempty",
			set:  Set{"one": {}, "two": {}, "three": {}, "four": {}, "five": {}},
			args: args{
				filter: func(item interface{}) bool {
					return len(item.(string)) > 3
				},
			},
			want: Set{"three": {}, "four": {}, "five": {}},
		},
		{
			name: "empty",
			set:  Set{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.set.Filter(tt.args.filter); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Set.Filter() = %v, want %v", got, tt.want)
			}
		})
	}
}
